        ...
        <?php if (!$tableExists): ?>
          <tr>
            <td colspan="10" class="text-center text-muted">
              `enquiries` table not found. Run the SQL query first.
            </td>
          </tr>
            ...
            <tr>
              ...
              <td>
                <div class="form-check form-switch">
                  <input
                    class="form-check-input enquiry-completed-checkbox"
                    type="checkbox"
                    id="completed_<?php echo (int)$row['id']; ?>"
                  >
                  <label
                    class="form-check-label completed-label"
                    for="completed_<?php echo (int)$row['id']; ?>"
                  >
                    Pending
                  </label>
                </div>
...
document.addEventListener('change', function (e) {
  if (e.target.classList.contains('enquiry-completed-checkbox')) {
    const label = e.target.closest('.form-check').querySelector('.completed-label');
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.enquiry-completed-checkbox').forEach(function (checkbox) {
    const id = checkbox.id;
    const savedValue = localStorage.getItem(id);
    const label = checkbox.closest('.form-check').querySelector('.completed-label');
...
document.addEventListener('change', function (e) {
  if (e.target.classList.contains('enquiry-completed-checkbox')) {
    ...
    if (e.target.checked) {
      ...
      localStorage.setItem(id, 'pending');
      label.textContent = 'Pending';
      label.classList.add('text-warning');
      label.classList.remove('text-success', 'fw-semibold');
    }
